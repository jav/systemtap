...
static inline int _stp_valid_stack_ptr(struct unwind_frame_info *info)
{
...
static int _stp_show_trace_unwind(struct unwind_frame_info *info, int verbose)
{
	int n = 0;

	while (unwind(info) == 0 && UNW_PC(info)) {
		if (_stp_valid_stack_ptr(info))
			break;
		n++;
		if (verbose) {
			_stp_print_char(' ');
			...
			_stp_printf ("0x%016lx ", UNW_PC(info));
